# impl: createImageBitmap の処理モデル実装計画

## 目的
- `createImageBitmap` の処理モデルを明確化し、仕様互換性・非破壊・性能向上を両立する。
- 互換性維持を最優先とし、破壊的変更は避ける。

## 背景の整理（要点）
- 仕様上 `ImageBitmap` の `[[BitmapData]]` は直接アクセスされないため、描画時に処理を実行する余地がある。
- 既存ブラウザ実装は、少なくとも **decode は即時**で、色変換や premultiply は **描画時にオンザフライ**で実施されることがある。
- Chromium の [static_bitmap_image_transform.cc](static_bitmap_image_transform.cc) にある `ApplyUsingPixmap` のように、**CPU パスでの実行時変換**も成立している（GPU フォールバックの余地）。
- Deno の現行実装は `createImageBitmap` 時に `DynamicImage` を確定しているため、見直し余地は後段処理にある。
- PR #28105 の `GPUQueue.copyExternalImageToTexture` は「描画/転送時に変換を適用する」方向の設計が含まれている。

## スコープ
### 取り込む範囲
- `ext/image` の `createImageBitmap` 実装を段階的に再構成（段階1）。
- `ImageBitmap` の内部表現を `Decoded` と「後段処理未適用」状態で保持し、消費側で `transform` 可能にする。
- Deno 内部で提供される描画 API（例: `GPUQueue.copyExternalImageToTexture`）で `transform` を実行。
...
- **後段処理（crop / resize / orientation / color space / premultiply）を利用時に適用**する。
- 後段処理の適用は `ImageBitmap::transform()` で統一。

### 内部表現の形
- `ImageBitmap` の内部を `Decoded` と「後段処理未適用」の2形態で保持する。
- 「後段処理未適用」が保持する最小情報:
  - `width` / `height`
  - `imageOrientation`
...
- 消費側 API（例: `copyExternalImageToTexture`）の責務:
  ...
  - 変換が必要なら transform-path を選択（段階1では `transform` 経由、将来は GPU 変換の余地を残す）。

### transform 実装の考慮点（CPU パス）
- GPU が使えない／使わない場合でも成立するよう、**CPU パスの変換を前提**に設計する。
- CPU パスでは `readPixels` 相当のコストが高いため、**`transform` の再実行を避ける**（`Decoded` への昇格キャッシュを必須とする）。
- CPU パスでの色変換・premultiply は既存 `image`/`image_ops` の処理で完結できる設計にする。

### transform 実装の考慮点（GPU パス・将来）
...
- 将来の GPU fast path 追加に備え、`transform()` は CPU パスの共通実装として維持する（消費側で分岐させる）。

## 消費側のトリガー（段階1）
- `Deno[Deno.internal].getBitmapData`（テスト用途）
- `GPUQueue.copyExternalImageToTexture`（PR #28105 相当）
- 将来的に `CanvasRenderingContext2D.drawImage` 相当が導入される場合は同様に `transform` を使う。
...
## 構造変更（想定）
- `ImageBitmap` を `Decoded` / 「後段処理未適用」状態に分ける enum で保持。
- `getData()` と `copyExternalImageToTexture` で `transform()` を呼び出す。
- `createImageBitmap` は `DynamicImage` を作成し、後段処理情報を「未適用状態」として保持。

## 互換性・エラー設計
- `sw/sh == 0` や `resizeWidth/resizeHeight == 0` は現行通り即時 reject。
- decode の失敗は `createImageBitmap` の Promise で reject（段階1で維持）。
- `transform` は `close()` 済みの場合にエラーを返す。
...
- `transform` / 消費側で扱うもの:
  ...
  - 後段処理の実行時にのみ発生し得る内部失敗（必要に応じて消費 API のエラーへ写像）
- 方針:
...
- 追加テスト（段階1）:
  - `createImageBitmap` が resolve し、`getData()` で `transform` が動くこと。
  - `copyExternalImageToTexture` が `transform` 後のデータを使うこと。
  - `close()` 後に消費 API を呼んだ場合のエラー確認。
  - 同一 `ImageBitmap` を複数回消費した際、2回目以降で再 `transform` せずキャッシュが使われること。
  - `copyExternalImageToTexture` の fast-path 条件を満たすケースで transform-path に落ちないこと（回帰防止）。

## リスクと緩和
- **互換性差分**: decode 即時を維持することで回避。
- **キャッシュメモリ**: `transform` 後のキャッシュは `close()` で明示破棄。
- **Deno外部ライブラリ**: Deno内部 API に限定して処理適用モデルを変更し、外部観測可能な挙動は維持。
...
- 将来的に対応する場合は、`transform()` を境界に「serialize 時に確定化」または「未適用情報を再現可能な形で転送」のいずれかを選べる設計にする。
- transfer 時の detach/close の意味は現在の `close()` と整合させる。

## 段階2以降（将来検討）
- ヘッダ解析のみで `width/height` を確定し decode を後段に回す案の検討。
- reject タイミング差分の互換性評価と文書化。
- GPU ターゲット経路に限定した transform の GPU fast path（readback 不要な場合にのみ適用）。
- GPU 側での色変換や premultiply のオンザフライ最適化。

## 進め方（作業順序）
1. `ImageBitmap` の内部表現を `Decoded` / 「後段処理未適用」状態に分離。
2. `transform()` を実装し、後段処理を集約。
3. `getData()` と `copyExternalImageToTexture` を `transform` 経由に変更。
4. 既存テストを通し、新規テスト追加。
5. 性能と互換性の評価メモを更新。

## 実装タスク（チケットレベル）
### T1: `ImageBitmap` の内部2形態追加
- 目的: `Decoded` / 「後段処理未適用」の2形態を導入し、後段処理を利用時に適用可能にする。
- 主な作業:
  - `ImageBitmap` 内部を enum で保持。
  - 「後段処理未適用」に必要なメタ情報を保持（crop/resize/色変換/premultiply）。
- 完了条件:
  - `ImageBitmap` が「後段処理未適用」を保持できる。
  - `width/height` は既存通り取得可能。

### T2: `transform()` 実装
- 目的: 未適用処理を仕様順序で適用し、`Decoded` に昇格させる。
- 主な作業:
  - crop → resize → orientation → color space → premultiply を順に適用。
  - `transform` 後のキャッシュ方針を実装（`Decoded` へ昇格）。
- 完了条件:
  - `transform` の再実行で再計算せず、同一結果を返す。

### T3: `createImageBitmap` の生成処理変更
- 目的: decode を即時に行いつつ、後段処理を `transform` に移す。
- 主な作業:
...
### T4: 消費側 API の `transform` 化
- 目的: 描画/転送時に後段処理を適用する。
- 主な作業:
  - `getData()` で `transform` を呼び出す。
  - `GPUQueue.copyExternalImageToTexture`（PR #28105 相当）で `transform` を呼び出す。
  - `copyExternalImageToTexture` に fast-path / transform-path の分岐を導入する。
- 完了条件:
  - 「後段処理未適用」状態の `ImageBitmap` を消費しても正しいデータが取得できる。
  - fast-path 条件を満たすケースで不要な変換処理を回避できる。

### T5: テスト追加・更新
- 目的: 処理モデル変更による挙動差分を検出。
- 主な作業:
  - `getData()` が `transform` を引き起こすことを検証。
...
## 差分ベースの TODO リスト
- `ext/image/bitmap.rs`
  - `ImageBitmap` を `Decoded` / 「後段処理未適用」状態に分離。
  - `transform()` 追加（後段処理を集約）。
  - `op_create_image_bitmap` の後段処理を `transform` に移管。
  - `getData()` が `transform` 経由でデータ取得するよう変更。
- `ext/webgpu/queue.rs`（PR #28105 の実装相当）
  - `copyExternalImageToTexture` で `transform` を呼び出す。
- `tests/unit/image_bitmap_test.ts`
  - 「後段処理未適用」状態での `getData()` / `close()` の挙動を追加検証。
